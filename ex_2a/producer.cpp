...
// The two processes communicate using the signal send

// execute wrtie action with rising edge of the clock if FIFO is not full(i.e. full == low)
// Certain value is outputted to d_out and wr_en is set to high
void producer::producer_proc() {
	unsigned char prod_data = 0;
	while(true) {
		wait();
...
// Set the signal send to HIGH triggering the other process
