...
void consumer::consumer_proc() {
	...
	while(true) {
		wait();
		// data vaild helps identify the correct clock edge for reading the FIFO data
		// corresponds to rd_en but delayed by one clock cycle
		if(data_valid.read() == true) {
		...
		// set rd_en to high after fetch is verified/asserted
		rd_en.write(fetch.read());
	}
}

// generate a pattern of read actions
// By asserting fetch, the other process is triggered (read operation)
