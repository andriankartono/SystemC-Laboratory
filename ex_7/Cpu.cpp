//this exercice focus on ip forwarding (processing the header)
...
void Cpu::processor_thread(void) {
	...
	while(true) {
		...
		while(packetReceived_interrupt.read()){
            ...
            startTransaction(TLM_READ_COMMAND, m_packet_descriptor.baseAddress, (unsigned char *) &m_packet_header, sizetoread);
            ...
            if(verifyHeaderIntegrity(m_packet_header)){
                ...
                startTransaction(TLM_WRITE_COMMAND, list[nextport], (unsigned char *) &m_packet_descriptor, sizeof(m_packet_descriptor));
                unsigned int newTTL = decrementTTL(m_packet_header);
                if(newTTL == 0){
                    //discard packet if TTL is 0
                    startTransaction(TLM_WRITE_COMMAND, DISCARD_QUEUE_ADDRESS, (unsigned char *) &m_packet_descriptor, sizeof(m_packet_descriptor));
                }else{
                    updateChecksum(m_packet_header);
                    //write header back to memory
                    startTransaction(TLM_WRITE_COMMAND, m_packet_descriptor.baseAddress, (unsigned char *) &m_packet_header, sizetoread);
                }
            }else{
